...
class DQN:
    ...
    def choose_action(self, state):
        ...
        if random.random() > self.epsilon(self.frame_idx):
            with torch.no_grad():
                ...
                q_values = self.policy_net(state.transpose(0, 1))
    ...
    def update(self):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, done_batch = self.memory.sample(
            self.batch_size)
        # transfer to tensor
        # print(f'state_batch: {state_batch.shape}, action_batch: {action_batch.shape}, reward_batch: {reward_batch.shape}, next_state_batch: {next_state_batch.shape}, done_batch: {done_batch.shape}')
        state_batch = torch.tensor(state_batch, device=self.device, dtype=torch.float).squeeze(-1)
        action_batch = torch.tensor(action_batch, device=self.device).unsqueeze(1)
        reward_batch = torch.tensor(reward_batch, device=self.device, dtype=torch.float).squeeze(-1)
        ...
        # print(f'q_values: {q_values.shape}, expected_q_values: {expected_q_values.unsqueeze(1).shape}')
